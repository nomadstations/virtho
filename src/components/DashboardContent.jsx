...
function DashboardContent() {
  ...
  const handleAddCourse = () => {
    // Navigate to dashboard with state/hash to indicate opening course modal 
    // Or let them handle it manually. Just linking to dashboard for now as per simple requirement.
    navigate('/dashboard');
  };
    ...
    >
      ...
      <motion.div variants={itemVariants} className="bg-white rounded-2xl p-8 shadow-sm border border-gray-100 flex flex-col md:flex-row items-center justify-between gap-6">
        ...
        <div className="flex flex-col gap-3 w-full md:w-auto">
          <Button onClick={handleAddCourse} className="bg-yellow-500 hover:bg-yellow-600 text-white px-6 py-6 rounded-xl shadow-md transition-all hover:shadow-lg w-full">
            <Plus className="w-5 h-5 mr-2" /> Add Course
          </Button>
          <Link to="/create-project" className="w-full">
            <Button className="bg-purple-600 hover:bg-purple-700 text-white px-6 py-6 rounded-xl shadow-md transition-all hover:shadow-lg w-full">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
          {quickAccess.map((item, index) => (
            <Link key={index} to={item.path}>
              <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm hover:shadow-md transition-all duration-300 group flex flex-col items-center text-center h-full">
