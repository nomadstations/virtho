...
import { ShoppingCart as ShoppingCartIcon, Menu, X, LayoutDashboard, ChevronDown, User, Building, Users, Zap } from 'lucide-react';
...
const Header = ({ setIsCartOpen }) => {
  ...
  // Acting on behalf state management
  const [actingContext, setActingContext] = useState('person'); // 'person', 'organization', 'group'
  ...
  const getNavLinkClass = (isActive, isMobile = false) => {
    ...
    const active = isActive ? 'bg-lavender-lightest text-lavender-primary shadow-sm' : 'text-gray-600 hover:bg-lavender-lightest hover:text-lavender-primary';
  ...
  // Get dynamic icon based on actingContext
  const getActingIcon = () => {
  ...
  const handleContextChange = (data) => {
    ...
    // TODO: Persist to localStorage/context
    // localStorage.setItem('actingContext', JSON.stringify(data));
  };

  return (
    <header className="header-full-width sticky top-0 z-20 w-full bg-white/95 backdrop-blur-md border-b border-gray-200 shadow-sm">
                  ...
                  return (
                    ...
                    >
                      <button className="header-dropdown-button">
                        {link.name} <ChevronDown className={`w-4 h-4 header-dropdown-chevron ${activeDropdown === index ? 'rotate-180' : ''}`} />
                      </button>
                          ...
                          >
                            {link.subLinks.map((subLink) => (
                              <NavLink
                                key={subLink.path}
                                to={subLink.path}
                                className={({ isActive }) => `header-dropdown-item ${isActive ? 'active' : ''}`}
            ...
            {isAuthenticated ? (
              ...
              <div className="hidden lg:flex items-center gap-2">
                <Link to={ROUTES.LOGIN}>
                  <Button variant="ghost" className="text-gray-700 hover:bg-lavender-lightest font-semibold px-5 h-11">
                    Login
                  </Button>
                </Link>
                          ...
                          >
                            {link.subLinks.map(subLink => (
                              <NavLink
                                ...
                                className={({ isActive }) => `header-mobile-dropdown-item ${isActive ? 'active' : ''}`}
                              >
                                {subLink.name}
                              </NavLink>
                            ))}
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col">
                          ...
                          <span className="text-sm font-semibold text-gray-800">{actingEntity}</span>
                        </div>
                      </div>
                    </div>
      ...
      <QuickActionsDropdown isOpen={isQuickActionsOpen} setIsOpen={setIsQuickActionsOpen} />
